...
// canvas.clientWidth / canvas.clientHeight: the size the browser is
// displaying the canvas, in CSS pixels. Natively the drawing buffer always
// matches the window, so the frame size is the displayed size.
fn canvas_client_size(_frame: &Frame) -> (f32, f32) {
    ...
    {
        (_frame.width as f32, _frame.height as f32)
    }
}

async fn run() {
    // The page's canvas is 400x300 with CSS stretching it to fill the page;
    // we leave auto_resize off so the drawing buffer keeps that resolution.
    let mut app = App::new("WebGPU Canvas width, height").await;
    ...
    let deg_to_rad = |d: f32| d * std::f32::consts::PI / 180.0;

    let translation = [150.0, 25.0];
    let rotation = deg_to_rad(0.0);
    let scale = [1.0, 1.0];
    ...
    app.run(RenderMode::Once, move |frame: &Frame| {
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
        {
            ...
            // canvas.clientWidth / canvas.clientHeight: the displayed size
            let (client_width, client_height) = canvas_client_size(frame);
            let matrix_value = &mut uniform_values[K_MATRIX_OFFSET..K_MATRIX_OFFSET + 12];
            mat3::projection(client_width, client_height, matrix_value);
