...
async fn run() {
    ...
    let pipeline = app
        ...
        .create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("storage buffer like uniform"),
            layout: None,
    ...
    let static_uniform_buffer_size = 4 * 4 + // color is 4 32bit floats (4bytes each)
        2 * 4 + // offset is 2 32bit floats (4bytes each)
        2 * 4; // padding
    let uniform_buffer_size = 2 * 4; // scale is 2 32bit floats (4bytes each)
    ...
    for i in 0..K_NUM_OBJECTS {
        ...
        let uniform_values = vec![0.0f32; uniform_buffer_size / 4];
