use wgpu_fun::{print, App, Frame, RenderMode};

async fn run() {
    let mut app = App::new("WebGPU Bind Group Layouts - rgba32float broken").await;
    ...
    // Creating this bind group fails validation: the `layout: None` (auto)
    // pipeline layout wants a filterable float texture but `rgba32float`
    // is not filterable. In the browser the error is just logged to the
    // console; native wgpu would panic on an uncaptured error, so we
    // capture it with an error scope and print it like the console would.
    ...
    // an invalid bind group can't be used to render
    let bind_group = error.is_none().then_some(bind_group);
