...
async fn run() {
    let mut app = App::new("WebGPU Scene Graph - Hand w/shot").await;
    ...
    app.run(RenderMode::Continuous, move |frame: &Frame| {
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
        {
            ...
            // In JavaScript `fireShot` ran when the button was clicked,
            // between frames. Here the page's Fire! button bumps the "fire"
            // counter setting and we notice inside the frame callback, after
            // the world matrices were updated, so we update the new node's
            // world matrix ourselves.
            ...
            if fire != last_fire {
                ...
                scene.nodes[node].local_matrix =
                    m4::translate(&scene.nodes[finger_tip].world_matrix, [0.0, 20.0, 0.0]);
                add_mesh(&mut meshes, node, K_SHOT_VERTICES, K_WHITE);
                let velocity = vec3::mul_scalar(
                    vec3::normalize(vec3::get_axis(&scene.nodes[finger_tip].world_matrix, 1)),
                    K_SHOT_VELOCITY,
                );
                shots.push(Shot {
                    node,
